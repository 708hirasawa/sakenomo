...
else if($_POST["search_type"] == 2)
{
	...
	$sql = "SELECT sakagura_name as sake_name, sakagura_read as sake_read, id as sake_id, pref, address, phone, url, SAKAGURA_J.date_added FROM SAKAGURA_J " .$condition." ORDER BY " .$orderby ." LIMIT ".$_POST["search_limit"];
	...
	else
	{
		...
		while($row = getnextrow($res))
		{
			$intime = gmdate("Y/m/d", $row["date_added"] + 9 * 3600);
			$result[] = array('sake_name' => $row["sake_name"], 'name' => $row["sake_read"], 'sake_id' => $row["sake_id"], 'pref' => $row["pref"], 'address' => $row["address"], 'phone' => $row["phone"], 'url' => $row["url"], 'filename' => $path, 'date_added' => $intime);
		}
...
else if($_POST["search_type"] == 4)
{
	...
	$sql = "SELECT username, fname, lname, bdate, email, phone, user_added_date FROM USERS_J " .$condition." ORDER BY user_added_date DESC"." LIMIT ".$_POST["search_limit"];
	...
	else
	{
		while($row = getnextrow($res))
		{
			...
			$result[] = array('username' => $row["username"], 'fname' => $row["fname"], 'lname' => $row["lname"], 'email' => $row["email"], 'phone' => $row["phone"], 'bdate' => $row["bdate"], 'profile_image' => $path, 'added_date' => $intime);
		}
