...
if($_POST["category"] == 1)
{
	...
	$sql  = 'SELECT			SAKE_J.sake_id AS id, sake_name, sake_read, SAKE_J.sake_rank AS rank, SAKE_J.write_update AS write_date, sakagura_name, special_name, sake_category, alcohol_level, jsake_level, rice_used, seimai_rate, postal_code, pref, address, SAKE_J.sake_id as observation, SAKE_J.sake_id as direct_sale, SAKE_J.sake_id as brand ';
	$sql .=	' FROM			SAKE_J, SAKAGURA_J ' .$condition1;
	$sql .= ' UNION ';
	$sql .= ' SELECT		SAKAGURA_J.id AS id, SAKAGURA_J.sakagura_name, SAKAGURA_J.sakagura_read, SAKAGURA_J.rank AS rank, SAKAGURA_J.date_updated AS write_date, null, null, null, null, null, null, null, postal_code, pref, address, observation, direct_sale, brand ';
	...
	$sql .= ' ORDER BY sake_read ASC LIMIT	'	   .$from .', ' .$to;
...
else if($_POST["category"] == 5)
{
	...
	else
	{
		while($row = getnextrow($res))
		{
			...
			$sql2 = "SELECT * FROM PROFILE_IMAGE WHERE contributor = '$contributor' AND status = 1";
			$res2 = executequery($db, $sql2);
			$image_path = "images/icons/noimage_user30.svg";

			if($rd = getnextrow($res2)) {
				$image_path = "images/profile/" .$rd["filename"];
			}
